...
// own may reach sh.
//
// ok is false when the caller must give up; every refusal here is a malformed
// invocation of gate itself (InvalidUsage), and has already been written to
// stderr.
func parseChdir(args []string, stderr io.Writer) (dir string, rest []string, ok bool) {
...
// malformed -C is the caller's mistake (InvalidUsage), a directory that is not
// there is the system's (Fatal). Collapsing the two would lose which one
// happened.
func enterable(dir string, stderr io.Writer) bool {
